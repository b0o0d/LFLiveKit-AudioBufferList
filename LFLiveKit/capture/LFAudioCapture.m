...
- (instancetype)initWithAudioConfiguration:(LFLiveAudioConfiguration *)configuration{
    if(self = [super init]){
        ...
        self.audioFormat = [[AVAudioFormat alloc] initWithStreamDescription:&desc];
...
static OSStatus handleInputBuffer(void *inRefCon,
    ...
    @autoreleasepool {
        ...
        if (!status) {
            ...
            if (source.delegate && [source.delegate respondsToSelector:@selector(captureOutput:audioBufferList:)]) {
                [source.delegate captureOutput:source audioBufferList:&buffers];
            }
        }
        return status;
    }
